...
use crate::balance::{
    increment_balance, increment_series, increment_series_balance, increment_supply, read_balance,
    read_native_token, read_series, read_supply, write_native_token,
};
use crate::data_type::{Metadata, Series};
use crate::events::{BuyEvent, ClaimEvent, CreateEvent};
use crate::metadata::{
    get_series_id, map_token_to_owner, map_token_to_series, read_metadata, read_name,
    read_owned_tokens, read_symbol, write_metadata, write_name, write_symbol,
};
use crate::owner::{
    read_creator, read_token_owner, write_creator, write_creator_curved, write_token_owner,
};
use crate::series::{
...
use soroban_sdk::{contract, contractimpl, log, symbol_short, Address, BytesN, Env, String, Vec};
...
impl NonFungibleTokenTrait for NonFungibleToken {
    ...
    fn transfer(_env: Env, _from: Address, _to: Address, _id: u128) {
        panic!("Can not transfer NFTs");
    }

    fn transfer_from(_env: Env, _from: Address, _to: Address, _id: u128) {
        panic!("Can not transfer NFTs");
    }
