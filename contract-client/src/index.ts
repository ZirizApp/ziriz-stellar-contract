...
export const networks = {
	testnet: {
		...
		contractId: 'CAH7ZYNYEVAPQW3MUHKRD6Z27E6AN64VTZR5S4LSOB7XA44JZ2LY2KGK',
	},
} as const
...
export type DataKey =
	...
	| { tag: 'Price'; values: readonly [u128] }
	| { tag: 'NativeToken'; values: void }
	| { tag: 'Series'; values: void }
	| { tag: 'SeriesSales'; values: readonly [u128] }
	| { tag: 'FanBasePrice'; values: readonly [u128] }
	| { tag: 'FanDecayRate'; values: readonly [u128] }
	| { tag: 'SumFanCut'; values: readonly [u128] }
	| { tag: 'FanCut'; values: readonly [u128, u128] }
	| { tag: 'Creator'; values: readonly [u128] }
	| { tag: 'CreatorCurved'; values: readonly [u128] }
	| { tag: 'Wasm'; values: void }

/**
    
    */
export type UserDataKey =
	| { tag: 'OwnedSeriesOrder'; values: readonly [string, u128] }
	| { tag: 'LastClaim'; values: readonly [string, u128] }
...
export class Contract {
	...
	constructor(public readonly options: ClassOptions) {
		this.spec = new ContractSpec([
			...
			'',
			'AAAAAgAAAAAAAAAAAAAAC1VzZXJEYXRhS2V5AAAAAAIAAAABAAAAAAAAABBPd25lZFNlcmllc09yZGVyAAAAAgAAABMAAAAKAAAAAQAAAAAAAAAJTGFzdENsYWltAAAAAAAAAgAAABMAAAAK',
	...
	private readonly parsers = {
		initialize: () => {},
		admin: (result: XDR_BASE64): string =>
			this.spec.funcResToNative('admin', result),
		numberOfSeries: (result: XDR_BASE64): u128 =>
			this.spec.funcResToNative('number_of_series', result),
		createSeries: () => {},
		seriesInfo: (result: XDR_BASE64): Series =>
			this.spec.funcResToNative('series_info', result),
		seriesSales: (result: XDR_BASE64): u128 =>
			this.spec.funcResToNative('series_sales', result),
		creatorOf: (result: XDR_BASE64): string =>
			this.spec.funcResToNative('creator_of', result),
		shareBalance: (result: XDR_BASE64): u128 =>
			this.spec.funcResToNative('share_balance', result),
		buy: () => {},
		claimShare: () => {},
		upgrade: () => {},
		ownerGet: (result: XDR_BASE64): Option<string> =>
			this.spec.funcResToNative('owner_get', result),
		ownerSet: () => {},
		redeploy: () => {},
	}
	...
	public readonly fromJSON = {
		...
		numberOfSeries: this.txFromJSON<
			ReturnType<(typeof this.parsers)['numberOfSeries']>
		>,
		createSeries: this.txFromJSON<
			ReturnType<(typeof this.parsers)['createSeries']>
		>,
		seriesInfo: this.txFromJSON<
			ReturnType<(typeof this.parsers)['seriesInfo']>
		>,
		seriesSales: this.txFromJSON<
			ReturnType<(typeof this.parsers)['seriesSales']>
		>,
		creatorOf: this.txFromJSON<ReturnType<(typeof this.parsers)['creatorOf']>>,
		shareBalance: this.txFromJSON<
			ReturnType<(typeof this.parsers)['shareBalance']>
		>,
		buy: this.txFromJSON<ReturnType<(typeof this.parsers)['buy']>>,
		claimShare: this.txFromJSON<
			ReturnType<(typeof this.parsers)['claimShare']>
		>,
	...
	initialize = async (
		...
		return await AssembledTransaction.fromSimulation({
			...
			args: this.spec.funcArgsToScVals('initialize', {
				...
				native_token: new Address(native_token),
			}),
			...options,
			...this.options,
			errorTypes: Errors,
	...
	createSeries = async (
		...
		return await AssembledTransaction.fromSimulation({
			...
			args: this.spec.funcArgsToScVals('create_series', {
	 ...
	 */
	shareBalance = async (
		{ account, series_id }: { account: string; series_id: u128 },
		options: {
			/**
			 * The fee to pay for the transaction. Default: 100.
			 */
		...
		return await AssembledTransaction.fromSimulation({
			...
			args: this.spec.funcArgsToScVals('share_balance', {
	 ...
	 */
	buy = async (
		{ buyer, series_id }: { buyer: string; series_id: u128 },
		options: {
			/**
			 * The fee to pay for the transaction. Default: 100.
			 */
		...
		return await AssembledTransaction.fromSimulation({
			method: 'buy',
			args: this.spec.funcArgsToScVals('buy', {
	...
	claimShare = async (
		...
		return await AssembledTransaction.fromSimulation({
			method: 'claim_share',
			args: this.spec.funcArgsToScVals('claim_share', {
				account: new Address(account),
				series_id,
			}),
			...options,
	...
	ownerSet = async (
		...
		return await AssembledTransaction.fromSimulation({
			method: 'owner_set',
			args: this.spec.funcArgsToScVals('owner_set', {
				new_owner: new Address(new_owner),
			}),
			...options,
			...this.options,
			errorTypes: Errors,
